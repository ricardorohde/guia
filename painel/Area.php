...
class Area extends AreaModel {
    ...
    public function indexController() {
        $this->db->url = Router::base() . "/painel/area";
        $this->db->paginate(25);
        $this->db->query = "SELECT * FROM area ORDER BY area_pos ASC, area_nome ASC";
        $this->db->query()->fetchAllObj();
        //tpl->data é enviado ao template area.html para listagem no foreach
        $this->tpl->data = $this->db->data;
        $this->tpl->tpl('admin/area.phtml')
    ...
    public function atualizar() {
        ...
        Router::redirect("$this->baseuri/painel/area/ok-now/");
    }
