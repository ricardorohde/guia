...
(function($){
	...
	Redactor.prototype = {
			...
			var handler = $.proxy(function()
			{
				...
				if (this.opts.imageGetJson !== false)
				{
					$.getJSON(this.opts.imageGetJson, $.proxy(function(data) {
						...
						{
		var img = $('<img src="' + val.thumb + '" rel="' + val.image + '" class="redactor_thumb" />');
			...
			this.modalInit(RLANG.image, 'image', 850, handler, true);
		...
		modalInit: function(title, url, width, handler, scroll)
		{
			...
			if (scroll === true)
			{
				$('#redactor_image_box').height(450).css('overflow', 'auto');
			}
